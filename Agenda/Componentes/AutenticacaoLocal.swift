...
//  Agenda
//
//  Created by Matheus Golke Cardoso on 24/03/21.
...
class AutenticacaoLocal: NSObject {
    ...
    func autorizaUsuario(completion:@escaping(_ autenticado:Bool)->Void){
        ...
        if contexto.canEvaluatePolicy(.deviceOwnerAuthentication, error: &error){
            contexto.evaluatePolicy(.deviceOwnerAuthentication, localizedReason: "é necessário autentucação para apagar um aluno") { (resposta, erro) in
                completion(resposta)
            }
        }
