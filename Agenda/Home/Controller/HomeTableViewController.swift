...
class HomeTableViewController: UITableViewController, UISearchBarDelegate, NSFetchedResultsControllerDelegate {
    ...
    @objc func abrirActionSheet(_ longPress:UILongPressGestureRecognizer){
        if longPress.state == .began{
            guard let alunoSelecionado = gerenciadoDeResultados?.fetchedObjects?[(longPress.view?.tag)!] else {return}
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let aluno = gerenciadoDeResultados?.fetchedObjects![indexPath.row] else {return celula}
            ...
            AutenticacaoLocal().autorizaUsuario { (autenticado) in
                if autenticado{
                    DispatchQueue.main.async {
                        guard let alunoSelecionado = self.gerenciadoDeResultados?.fetchedObjects![indexPath.row] else {return}
    ...
    @IBAction func buttonCalculaMedia(_ sender: UIBarButtonItem) {
        guard let listaDeAlunos = gerenciadoDeResultados?.fetchedObjects else {return}
